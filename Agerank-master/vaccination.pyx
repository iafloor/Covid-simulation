...
def boosters(peoplep, startagep, weekp):
    ...
    for i in range(numberOfBoosters):
        ...
        person = people[startage]
        while perseon.weekOfVaccination < 0 or \
        person.susceptible == 0 or \
        week + 46 - person.weekOfVaccination < 12:
            startage -= 1
            if startage < 1 :
               return people, N-1
            person = people[startage]
        person.weekOfVaccination = week
        people[startage] = person
        startage -= 1
...
cdef list vaccinate(list people, int time ):
